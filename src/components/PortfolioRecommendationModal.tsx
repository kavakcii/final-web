...
import { X, Brain, TrendingUp, Sparkles, LineChart } from "lucide-react";
...
export function PortfolioRecommendationModal({ isOpen, onClose, data, userName, investmentAmount }: ModalProps) {
  ...
  const simulations = useMemo(() => {
    ...
    const principal = parseFloat(cleanAmount) || 10000; // Default 10K if nothing parsed
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4 z-[100] pointer-events-none">
            <motion.div
              ...
              className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden pointer-events-auto flex flex-col max-h-[90vh]"
            >
              {/* Header */}
              <div className="flex items-center justify-between p-6 md:px-10 border-b border-slate-100 bg-slate-50/80">
                ...
                <button 
                  onClick={onClose}
                  className="w-10 h-10 flex items-center justify-center rounded-full bg-white hover:bg-slate-200 shadow-sm border border-slate-100 text-slate-500 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
