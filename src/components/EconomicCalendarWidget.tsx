...
import { Calendar, Globe2, Loader2 } from "lucide-react";
...
export function EconomicCalendarWidget() {
    ...
    const [filter, setFilter] = useState<'all' | 'TR' | 'US' | 'EU' | 'GB'>('all');
    ...
    const filteredEvents = events.filter(e => {
        if (filter !== 'all') {
            ...
            if (filter === 'GB' && e.country !== 'GB' && e.country !== 'GBP') return false;
        }
        return true;
    });
        ...
        <div className="w-full bg-[#090a0d] border border-zinc-800/80 rounded-3xl p-6 shadow-2xl text-zinc-100 font-sans flex flex-col justify-between transition-all">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-zinc-800/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-0.5">
                                Türkiye Saati ile Anlık Makroekonomik Veriler
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 bg-zinc-900 p-1 rounded-2xl border border-zinc-800">
                        <button
                            ...
                            className={`px-3 py-1 text-[10px] font-extrabold rounded-xl transition-all ${filter === 'all' ? 'bg-blue-600 text-white shadow-md' : 'text-zinc-400 hover:text-white'}`}
                        >
                            Tümü
                        </button>
                        <button
                            onClick={() => setFilter('TR')}
                            className={`px-2.5 py-1 text-[10px] font-extrabold rounded-xl transition-all ${filter === 'TR' ? 'bg-blue-600 text-white shadow-md' : 'text-zinc-400 hover:text-white'}`}
                        >
                            🇹🇷 TR
                        </button>
                        <button
                            onClick={() => setFilter('US')}
                            className={`px-2.5 py-1 text-[10px] font-extrabold rounded-xl transition-all ${filter === 'US' ? 'bg-blue-600 text-white shadow-md' : 'text-zinc-400 hover:text-white'}`}
                        >
                            🇺🇸 US
                        </button>
                        <button
                            onClick={() => setFilter('EU')}
                            className={`px-2.5 py-1 text-[10px] font-extrabold rounded-xl transition-all ${filter === 'EU' ? 'bg-blue-600 text-white shadow-md' : 'text-zinc-400 hover:text-white'}`}
                        >
                            🇪🇺 EU
                        </button>
                        <button
                            onClick={() => setFilter('GB')}
                            className={`px-2.5 py-1 text-[10px] font-extrabold rounded-xl transition-all ${filter === 'GB' ? 'bg-blue-600 text-white shadow-md' : 'text-zinc-400 hover:text-white'}`}
                        >
                            🇬🇧 UK
                        </button>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-zinc-800/80 text-[10px] font-black text-zinc-400 uppercase tracking-wider pb-2">
                                    <th className="py-2.5 px-3 w-16 text-zinc-400">Saat (TSİ)</th>
            ...
            <div className="mt-4 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-[10px] font-bold text-zinc-400">
                <span className="flex items-center gap-1.5">
                    ...
                    <span>Türkiye 🇹🇷 | ABD 🇺🇸 | Euro Bölgesi 🇪🇺 | İngiltere 🇬🇧</span>
                </span>
                <span className="text-blue-400 font-extrabold">TSİ (UTC+3) Anlık Akış</span>
            </div>
        </div>
    );
}
