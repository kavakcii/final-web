...
const ShaderCanvas = () => {
    ...
    return <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full block z-0 bg-white" />;
};
...
export const PricingCard = ({
    ...
    const isDarkTheme = isPopular; // Popular card gets the dark theme
  ...
  `;
    ...
    return (
        <div className={cardClasses.trim()}>
            {isPopular && (
                <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-blue-500 text-white shadow-md">
                    En Popüler
                </div>
            )}
            <div className="mb-3">
                <h2 className={`text-[32px] font-bold tracking-tight ${titleColor}`}>{planName}</h2>
                <p className={`text-[14px] mt-1 font-sans ${descColor}`}>{description}</p>
            </div>
            <div className="my-6 flex items-baseline gap-1 py-2">
                <span className={`text-[36px] font-bold ${priceColor}`}>{price}₺</span>
                <span className={`text-[14px] font-sans ${descColor}`}>/ay</span>
            </div>
            <div className={`card-divider w-full mb-5 h-px ${dividerColor}`}></div>
            <ul className={`flex flex-col gap-3 text-[14px] mb-6 font-sans ${featureColor}`}>
                {features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-2">
                        <CheckIcon className={`${checkColor} w-4 h-4`} /> {feature}
                    </li>
                ))}
            </ul>
            <RippleButton className={processedButtonClasses.trim()}>{buttonText}</RippleButton>
        </div>
    );
};
    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-5xl mx-auto px-4 py-8">
                ...
                <div className="flex flex-col md:flex-row gap-8 md:gap-6 justify-center items-stretch w-full">
